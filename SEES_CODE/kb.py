...
if((gamma /  (2*(n-1)) ) > theta):
	...
	for i in range(0,int(math.sqrt(int(Z)))):#for y axis
	    for j in range(0,int(math.sqrt(int(Z)))):#for x axis
	        for p in getpoints(j*20,i*20,(j+1)*20,(i+1)*20,j*20+1+200*i):
	        	zones_objects.append(nd.Node(p[0][0],p[0][1],p[1],p[2]))
	...
	no_lbs_on_each_side = int(B/4)
	Lbs_side = (L+2*20)*4
	distance_btw_lbs = int(Lbs_side/B)
	...
	y = [-50, L+50, L+50, -50]
	ax1.fill(x,y,'y')
	lb_poiints = get_rand_uniform_points_LBS(-50,-20,-50,L+50)
	for i in range(len(lb_poiints[0])):
		if lbs_count <= B:
			print(lbs_count)
			LBS.append(L_B_S.LBS(lb_poiints[0][i],lb_poiints[1][i],lbs_count))
			lbs_count+=1
	# print(lbs_count)
	...
	ax1.fill(x,y,'y')
	lb_poiints = get_rand_uniform_points_LBS(-20,L+50,L+20,L+50)
	for i in range(len(lb_poiints[0])):
		if lbs_count <= B:
			print(lbs_count)
			LBS.append(L_B_S.LBS(lb_poiints[0][i],lb_poiints[1][i],lbs_count))
	...
	y = [ L+50,-50 ,-50,L+50]
	ax1.fill(x,y,'y')
	lb_poiints = get_rand_uniform_points_LBS(L+20,L+50,-50,L+50)
	for i in range(len(lb_poiints[0])):
		if lbs_count <= B:
			print(lbs_count)
			LBS.append(L_B_S.LBS(lb_poiints[0][i],lb_poiints[1][i],lbs_count))
	...
	y = [ -50,-20 ,-20,-50]
	ax1.fill(x,y,'y')
	lb_poiints = get_rand_uniform_points_LBS(-20,L+20,-50,-20)
	for i in range(len(lb_poiints[0])):
		if lbs_count <= B:
			print(lbs_count)
			LBS.append(L_B_S.LBS(lb_poiints[0][i],lb_poiints[1][i],count))
			lbs_count+=1
	# print(lbs_count)
