...
def updatefig(i):
    ...
    if (not(network.is_Network_alive())):
    	...
    	pass
    l = 0
    ###############  random way points moments and getting their ne poitions  #######################
    #------------------------------------mobility -----------------------------------------------#
    for i in network.get_zones_in_network():
        l+=1
        t = i.zone_is_alive()
        # cardinality of nodes in a zone
        (x_init,y_init) = i.get_init_coord()
        Nalive = t[0]
        # set of alive nodes
        HNsalive = t[1]
        # get each alive nodes
        dist_m = (D/4)
        for o in HNsalive:
            (xcoord,ycoord) = o.getlocation()
            if o.energyformoving():# this is for energy cost while moving
            	continue
            # f.write("old :"+str(xcoord)+","+str(ycoord)+",")
            degrees = randint(0,360)
            angle = degrees* (PI /180.0)
            x_new, y_new = int(xcoord+math.cos(angle)*dist_m), int(ycoord+math.sin(angle)*dist_m)
            # boundary condition of x
            while(not ((x_new > x_init and x_new < (x_init+D)) and (y_new > y_init and y_new < (y_init+D))) ):
                degrees = randint(0,360)
                angle = degrees* (PI /180.0)
                x_new = int(xcoord+math.cos(angle)*dist_m)
                y_new =  int(ycoord+math.sin(angle)*dist_m)
            o.set_location(x_new,y_new)
    ...
    plt.axis([-60, L+60, -60, L+60])#defining axis x and y
